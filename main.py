...
class AccessPage(webapp2.RequestHandler):
    def get(self):
        ...
        if self.request.get('oauth_token') and self.request.get('oauth_token_secret'):
            access_token = access({'oauth_token': self.request.get('oauth_token'),
...
class UploadPage(webapp2.RequestHandler, webapp2.Request):
    def post(self):
    ...
    def get(self):
        ...
        if self.request.get('oauth_token') and self.request.get('oauth_token_secret') and \
           self.request.get('short') and self.request.get('name'):
            token = oauth2.Token(self.request.get('oauth_token'), self.request.get('oauth_token_secret'))
            url = sign("https://api.dropbox.com/1/shares/sandbox/%s?%s" % (self.request.get('name'),
...
app = webapp2.WSGIApplication([
    ...
    ('/upload', UploadPage),
    ('/share', SharePage),
    webapp2.Route('/', webapp2.RedirectHandler, defaults = {'_uri': 'https://github.com/pinting/JamCrop'})
], debug = False)
